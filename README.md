...
- **Look:** two inks (navy and sage) on paper, like a two-colour print. Colours live
  as CSS variables in `src/index.css`, so light and dark mode share one system.
- **Type:** Newsreader for display and Source Sans 3 for body, both self-hosted
  through `@fontsource-variable` packages (no Google Fonts request).
- **Illustrations:** all drawn as inline SVG in `src/art.jsx`. Each project has its
  own scene; hover a project to snap the sage layer into register.
- **Headline:** edit `HERO_LINES` in `src/Portfolio.jsx` (each line ends in a glyph).
- **Motion:** one load sequence (headline rises, glyphs draw in). Everything respects
